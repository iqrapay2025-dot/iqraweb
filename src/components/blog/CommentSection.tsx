...
import { Heart, MessageCircle, Share2, Link2, Send, Trash2, MoreHorizontal, MapPin } from 'lucide-react';
...
export function CommentSection({ postSlug, postTitle }: CommentSectionProps) {
      ...
      setComments([
        {
          ...
          content: 'MashaAllah! This is very beneficial knowledge. JazakAllahu Khayran for sharing.',
        ...
        },
        {
          ...
          avatar: 'MA',
          location: 'London, UK',
          content: 'Alhamdulillah, this platform is making it easier to learn our Deen. May Allah reward the team!',
  ...
  const renderComment = (comment: Comment, parentId?: string) => (
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className={`${parentId ? 'ml-12' : ''} border-b border-border last:border-0`}
    >
      <div className="flex gap-3 py-4 hover:bg-muted/20 transition-colors px-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-sm hover:underline cursor-pointer">{comment.author}</span>
            {comment.location && (
          ...
          <div className="flex items-center gap-6 text-muted-foreground">
            ...
            >
              <Heart
                className={`h-4 w-4 transition-all ${
                  comment.liked ? 'fill-red-500 text-red-500 scale-110' : 'group-hover:scale-110'
                }`}
              />
              {comment.likes > 0 && (
            ...
            <DropdownMenu
              ...
              onOpenChange={(open: boolean) => setShareMenuOpen(open ? comment.id : null)}
            >
